...
Loess_profile11 <- ggplot(MDF, aes(x = nr_point, y = profile11)) +
	geom_point(aes(x = nr_point, y = profile11, colour = "Samples", shape = "Samples"), show.legend=TRUE) +
...
#	scale_size_manual(values = c(profile11 = 1)) +
	labs(title="马里亚纳海沟。剖面11。Mariana Trench, Profile Nr.11.", 
...
ggsave("Loess_Profile11.pdf", device = cairo_pdf, fallback_resolution = 300)