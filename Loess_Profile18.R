...
Loess_profile18 <- ggplot(MDF, aes(x = nr_point, y = profile18)) +
	geom_point(aes(x = nr_point, y = profile18, colour = "Observation points", shape = "Observation points"), show.legend=TRUE) +
...
#	scale_size_manual(values = c(profile11 = 1)) +
	labs(title="Mariana Trench, Profile Nr.18", 
...
ggsave("Loess_Profile18.pdf", device = cairo_pdf, fallback_resolution = 300, width = 210, height = 297, units = "mm")