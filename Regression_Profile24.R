...
# шаг-6. рисуем один график (здесь: профиль №24) по кривой лесса с доверительными интервалами и квантилями
Loess_profile24 <- ggplot(MDF, aes(x = nr_point, y = profile24)) +
	geom_point(aes(x = nr_point, y = profile24, colour = "Samples", shape = "Samples"), show.legend=TRUE) +
	geom_smooth(aes(x = nr_point, y = profile24, colour = "Loess method"), method = loess, se = TRUE, span = .4, size=.3, linetype = "solid", show.legend=TRUE) +
	geom_smooth(aes(x = nr_point, y = profile11, colour = "Glm method"), method = glm, se = TRUE, span = .4, size=.4, linetype = "solid", show.legend=TRUE) +
	geom_smooth(aes(x = nr_point, y = profile24, colour = "Lm method"), method = lm, se = TRUE, size=.3, linetype = "solid", show.legend=TRUE) +
	geom_quantile(aes(x = nr_point, y = profile24, colour = "Quantiles"), linetype = "solid", show.legend=TRUE) +
	xlab("Observations") + 
	ylab("Depths, m") +
